...
CREATE TABLE IF NOT EXISTS files (
    ...
    parent_id INT UNSIGNED NULL REFERENCES files(id) ON DELETE CASCADE,
    status ENUM ('created', 'deleted', 'pending') NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS group_members (
    group_id INT UNSIGNED NOT NULL REFERENCES user_groups(id) ON DELETE CASCADE,
    user_id INT UNSIGNED NOT NULL REFERENCES users(id) ON DELETE CASCADE,
