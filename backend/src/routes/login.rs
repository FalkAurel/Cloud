...
impl<T: Sized> SendSliceAcrossThreads<T> {
    pub fn from(value: &[T]) -> Self {
        Self {
            ptr: AtomicPtr::new(value.as_ptr().cast_mut()),
            len: value.len(),
        }
    }
...
pub async fn login(login_request: Json<UserLoginRequest<'_>>) -> (Status, &'static str) {
    ...
    match result {
        Ok(Some(row)) => {
            let login_password: SendSliceAcrossThreads<u8> =
                SendSliceAcrossThreads::from(login_request.0.password.as_bytes());

            let task: JoinHandle<(Status, &str)> =
                spawn_blocking(move || -> (Status, &'static str) {
